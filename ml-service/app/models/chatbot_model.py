...
# Suppress warnings about quantization_config
...
class ChatbotMLModel:
    ...
    def load_model(self):
        ...
        if os.path.exists(model_file) and os.path.exists(tokenizer_file):
            try:
                # Define compatibility layer for loading
                class EmbeddingCompat(keras.layers.Embedding):
                ...
                with keras.utils.custom_object_scope({'Embedding': EmbeddingCompat}):
                    self.model = keras.models.load_model(model_file)
                
                self.tokenizer = joblib.load(tokenizer_file)
                self.label_encoder = joblib.load(encoder_file)
                print("✅ Modèle chatbot ML chargé avec compatibilité")
    ...
    def train_model(self):
        ...
        try:
            ...
            print(f"❌ Fichier intents.json non trouvé: {e}")
    ...
    def _create_minimal_model(self):
        """Créer un modèle minimal pour le chatbot"""
        # Créer un modèle simple avec des classes par défaut
        self.label_encoder = LabelEncoder()
        default_tags = ['greeting', 'help', 'program', 'fallback']
        self.label_encoder.fit(default_tags)
        ...
        model = keras.Sequential([
            layers.Embedding(100, 32, input_length=20),
